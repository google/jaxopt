...
def _split_and_pack_like(x: jnp.ndarray, tree: Any):
  ...
  return jax.tree_util.tree_map(
      lambda y, p: jnp.reshape(y, p.shape),
...
def _find_cauchy_point(
  ...
  start_ind = jnp.argmax(
      jnp.concatenate(
          [active_set_mask_sorted, jnp.ones([1]).astype(jnp.bool_)], axis=0
      )
  )
...
class LBFGSB(base.IterativeSolver):
  ...
  def update(
    ...
    fill_diag_syt = jnp.where(
        state.num_updates > 0, s_dot_yt[prev_ind, prev_ind], 1.0
    )
    ...
    # (m, m) diagonal
    diag = -1.0 * jnp.diag(jnp.diag(s_dot_yt)) + diag_adj

    s_dot_st = jnp.matmul(s_history_flat, jnp.transpose(s_history_flat))
    fill_diag_sst = jnp.where(
        state.num_updates > 0, s_dot_st[prev_ind, prev_ind], 1.0
    )
    ...
    m_inv = jnp.concatenate(  # Equation 3.4 of [1].
        [
            ...
            jnp.concatenate(
                [lower_tril, sst_adj + state.theta * s_dot_st], axis=1
            ),
        ],
        axis=0,
    )
    ...
    use_linesearch = (
        not isinstance(self.stepsize, Callable) and self.stepsize <= 0.0
    )
    if use_linesearch:
    ...
    (new_s_history, new_y_history, new_theta, new_num_updates) = jax.lax.cond(
        curvature > 0.0,
  ...
  def __post_init__(self):
    ...
    # FIXME: the maximal stepsize should be selected at the start of
    # linesearch_solver.run. Currently the max_stepsize does not match the current
    # bound possible at the iterate. For the zoom linesearch, one needs a large
    # value of max_stepsize to be able to satisfy the curvature condition.
