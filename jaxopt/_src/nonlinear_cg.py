...
class NonlinearCG(base.IterativeSolver):
  ...
  Attributes:
    ...
    has_aux: whether ``fun`` outputs auxiliary data or not. If ``value_and_grad
      == False``, the output should be ``value, aux = fun(...)``. If
      ``value_and_grad == True``, the output should be ``(value, aux), grad =
      fun(...)``. The auxiliary outputs are stored in ``state.aux``.
    maxiter: maximum number of proximal gradient descent iterations.
    tol: tolerance of the stopping criterion.
    method: which variant to calculate the beta parameter in Nonlinear CG.
      "polak-ribiere", "fletcher-reeves", "hestenes-stiefel" (default:
      "polak-ribiere")
  ...
  def init_state(self, init_params: Any, *args, **kwargs) -> NonlinearCGState:
  ...
  def update(
      self, params: Any, state: NonlinearCGState, *args, **kwargs
  ) -> base.OptStep:
    ...
    if self.method == "polak-ribiere":
      ...
      raise ValueError(
          "method argument should be either 'polak-ribiere', "
          "'fletcher-reeves', or 'hestenes-stiefel'."
      )

    new_descent_direction = tree_add_scalar_mul(
        tree_scalar_mul(-1, new_grad), new_beta, descent_direction
    )
