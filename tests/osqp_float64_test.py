...
def _from_boxosqp_to_osqp(Q, c, A, l, u):
...
class BoxOSQP64Test(jtu.JaxTestCase):
...
class OSQPTest(jtu.JaxTestCase):
  ...
  def test_qp_eq_and_ineq(self, matvec_A, matvec_G):
    Q = 2 * jnp.array([[2.0, 0.5], [0.5, 1]], dtype=jnp.float64)
    c = jnp.array([1.0, 1.0], dtype=jnp.float64)
    A = jnp.array([[1.0, 1.0]], dtype=jnp.float64)
    b = jnp.array([1.0], dtype=jnp.float64)
    G = jnp.array([[-1.0, 0.0], [0.0, -1.0]], dtype=jnp.float64)
    h = jnp.array([0.0, 0.0], dtype=jnp.float64)
  ...
  def test_projection_simplex(self):
    def _projection_simplex_qp(x, s=1.0):
      Q = jnp.eye(len(x), dtype=jnp.float64)
      A = jnp.array([jnp.ones_like(x)], dtype=jnp.float64)
      b = jnp.array([s], dtype=jnp.float64)
      G = -jnp.eye(len(x), dtype=jnp.float64)
      h = jnp.zeros_like(x, dtype=jnp.float64)
