...
class PicardIterationTest(jtu.JaxTestCase):
  ...
  def test_geometric_decay(self):
    ...
    tol = 1e-5
    sol, state = PicardIteration(f, maxiter=8000, tol=tol).run(x0)
  ...
  def test_sin_fixed_point(self):
    ...
    tol = 1e-6
    sol, state = PicardIteration(f, maxiter=100, tol=tol).run(x0)
    self.assertLess(state.error, tol)
    sol_norm = tree_l2_norm(sol)
  ...
  def test_cos_fixed_point(self):
    ...
    tol = 1e-2
    sol, state = PicardIteration(f, maxiter=100000, tol=tol).run(x0, theta)
    self.assertLess(state.error, tol)
  ...
  def test_has_aux(self):
    ...
    tol = 2e-2
    aa = PicardIteration(f, maxiter=400*1000, tol=tol, has_aux=True)
  ...
  def test_simple_grads(self):
    ...
    theta = jnp.array([0., jnp.pi / 2])
    aa = PicardIteration(f, maxiter=4*1000, tol=1e-6)
    def solve_run(args, kwargs):
      return aa.run(x0, *args, **kwargs)[0]
    check_grads(solve_run, args=([theta], {}), order=1, modes=['rev'], eps=None)
    aa_unrolled = PicardIteration(f, maxiter=4*1000, tol=1e-6, implicit_diff=False)
    def solve_run(args, kwargs):
  ...
  def test_grads_flat_landscape(self):
    ...
    aa = PicardIteration(f, maxiter=1000, tol=1e-6, has_aux=True)
    def solve_run(args, kwargs):
      return aa.run(x0, *args, **kwargs)[0][0]
  ...
  def test_affine_contractive_mapping(self):
    ...
    def f(x, M, b):
      return M @ x + b
    tol = 1e-6
    aa = PicardIteration(f, maxiter=100, tol=tol)
    x0 = jnp.zeros_like(b)
